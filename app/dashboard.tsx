...
function Pagination({currentPage,totalItems,pageSize=15,onPageChange}:{currentPage:number;totalItems:number;pageSize?:number;onPageChange:(page:number)=>void}) {
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
  if (totalItems <= pageSize) return null;
  const start = (currentPage - 1) * pageSize + 1;
  const end = Math.min(totalItems, currentPage * pageSize);
  return <div className="pagination">
    <span>Hiển thị <strong>{start}</strong>–<strong>{end}</strong> trong tổng số <strong>{totalItems}</strong> mục</span>
    <div>
      <button type="button" disabled={currentPage <= 1} onClick={() => onPageChange(currentPage - 1)}>← Trước</button>
      <span style={{ display: "inline-flex", alignItems: "center", padding: "0 8px", fontSize: "9.5px", fontWeight: 700 }}>Trang {currentPage} / {totalPages}</span>
...
function PurchaseTable({orders,onOpen,compact=false,selectedIds=[],onToggle}:{orders:Row[];onOpen:(order:Row)=>void;compact?:boolean;selectedIds?:string[];onToggle?:(id:string)=>void}) {
  ...
  const pagedOrders = compact ? orders : orders.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  const selectable=Boolean(onToggle);
  return <>
    <div className="table-wrap"><table className={compact?"compact-table":""}><thead><tr>{selectable&&<th>Gom</th>}<th>Mã / Ngày</th><th>Nguồn</th><th>Sản phẩm</th><th>Tiến độ nhận</th><th>Tiền hàng</th><th>Ship</th><th>Tổng chi</th><th>Còn trả NCC</th><th>Trạng thái</th><th></th></tr></thead><tbody>{pagedOrders.length?pagedOrders.map((order)=>{
      ...
      return <tr key={id} className={effectivePurchaseStatus(order)==="MERGED"?"merged-purchase-row":""}>{selectable&&<td><input className="purchase-select" type="checkbox" aria-label={`Chọn ${String(order.code)} để gom`} checked={selectedIds.includes(id)} disabled={!canSelect} onChange={()=>onToggle?.(id)} /></td>}<td><strong>{String(order.code)}</strong><small className="cell-note">{dateValue(order.order_date)}</small>{Number(order.consolidated_from_count)>0&&<small className="cell-note positive">Đơn tổng từ {qty(order.consolidated_from_count)} đơn nháp</small>}{mergedIntoCode&&<small className="cell-note">Gom vào {mergedIntoCode}</small>}</td><td><span className="source-cell">{String(order.tracking_sources||order.supplier||"Chưa xác định")}</span></td><td className="wide-cell">{rows(order.items).filter((item)=>item.fulfillment_type!=="ATTACHED_BOX").map((item)=>`${item.name} ×${item.ordered_qty}`).join(" · ")||"—"}</td><td><strong>{qty(order.received_qty)} / {qty(order.ordered_qty)}</strong><div className="progress"><span style={{width:`${Math.min(100,Number(order.ordered_qty)?Number(order.received_qty)/Number(order.ordered_qty)*100:0)}%`}} /></div></td><td>{money(order.total_amount)}</td><td>{money(order.ship_cost)}</td><td><strong>{money(Number(order.total_amount||0)+Number(order.ship_cost||0))}</strong></td><td><strong className={Number(order.outstanding)>0?"negative":"positive"}>{money(order.outstanding)}</strong></td><td><StatusTag status={effectivePurchaseStatus(order)} purchase /></td><td><button className="edit-button" onClick={()=>onOpen(order)}>Mở</button></td></tr>;
    }):<tr><td colSpan={selectable?11:10}><Empty text="Chưa có đơn nhập phù hợp." /></td></tr>}</tbody></table></div>
    {!compact && <Pagination currentPage={currentPage} totalItems={totalItems} pageSize={pageSize} onPageChange={setPage} />}
  </>;
}

function SalesTable({orders,onOpen}:{orders:Row[];onOpen:(order:Row)=>void}) {
  const [page, setPage] = useState(1);
  const pageSize = 15;
  const totalItems = orders.length;
  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
  ...
  return <>
    <div className="table-wrap"><table><thead><tr><th>Mã / Ngày</th><th>Khách hàng</th><th>Sản phẩm</th><th>Nguồn</th><th>Tổng khách trả</th><th>Đã thu</th><th>Lãi</th><th>Giữ kho</th><th>Trạng thái</th><th></th></tr></thead><tbody>{pagedOrders.length?pagedOrders.map((order)=><tr key={String(order.id)}><td><strong>{String(order.code)}</strong><small className="cell-note">{dateValue(order.order_date)}</small></td><td>{String(order.customer)}<small className="cell-note">{String(order.phone||"")}</small></td><td className="wide-cell">{rows(order.items).map((item)=>`${fullProductName(item)} ×${item.quantity}`).join(" · ")}</td><td><span className="source-cell">{String(order.tracking_source||order.source_supplier||"Chưa xác định")}</span></td><td>{money(orderCustomerTotal(order))}<small className="cell-note">{order.ship_payer==="RECIPIENT"?"Người nhận trả ship":"Người bán trả ship"}</small></td><td>{money(order.paid_amount)}</td><td>{order.workflow_status==="WAITING_STOCK"?<span className="waiting-cost">Chờ giá vốn</span>:<strong className={Number(order.display_profit)>=0?"positive":"negative"}>{money(order.display_profit)}</strong>}</td><td>{Number(order.reserved_qty)>0?<span className="tag pending">{qty(order.reserved_qty)} SP</span>:order.workflow_status==="WAITING_STOCK"?<small className="cell-note">Chưa giữ</small>:"—"}</td><td><StatusTag status={String(order.workflow_status)} /></td><td><button className="edit-button" onClick={()=>onOpen(order)}>Mở</button></td></tr>):<tr><td colSpan={10}><Empty text="Không tìm thấy đơn hàng." /></td></tr>}</tbody></table></div>
    <Pagination currentPage={currentPage} totalItems={totalItems} pageSize={pageSize} onPageChange={setPage} />
  </>;
}
